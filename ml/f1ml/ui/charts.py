...
def accuracy_bars(metrics: dict) -> go.Figure:
  methods = ["pole", "champ", "logreg", "rf"]
  labels = ["Pole", "Champ leader", "Logistic reg.", "Random forest"]
  train = [metrics[f"train_{m}_hit"] * 100 for m in methods]
  test = [metrics[f"test_{m}_hit"] * 100 for m in methods]
  fig = go.Figure()
